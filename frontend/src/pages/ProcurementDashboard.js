...
import { Package, LogOut, Clock, CheckCircle, RefreshCw, FileText, ShoppingCart, Truck, Eye, Download, Calendar, Filter, Check, AlertCircle, Plus, Users, X, Edit, DollarSign, BarChart3, Trash2, KeyRound, Loader2, Search, TrendingUp, Menu, Settings, ChevronLeft, Upload } from "lucide-react";
                            ...
                            <TableRow key={order.id} className={order.status === "delivered" ? "bg-emerald-50/30" : ""}>
                              ...
                              <TableCell className="text-center">
                                <div className="flex gap-1 justify-center">
                                  ...
                                  <Button size="sm" variant="ghost" onClick={() => handleDeleteOrder(order.id)} className="h-8 w-8 p-0" title="حذف أمر الشراء"><Trash2 className="w-4 h-4 text-red-600" /></Button>
                                </div>
                              </TableCell>
                            </TableRow>
                          ))}
