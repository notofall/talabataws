...
agent_communication:
  ...
  - agent: "testing"
    message: "CRITICAL ARABIC PDF EXPORT ISSUE DISCOVERED: Tested Arabic text in PDF export as requested. Successfully completed full workflow: logged in as manager1@test.com → navigated to أوامر الشراء section → clicked eye icon → opened purchase order dialog (تفاصيل أمر الشراء) → clicked تصدير PDF button → downloaded امر_شراء_34c949d8.pdf (3058 bytes). PDF file is structurally valid but ARABIC TEXT IS NOT RENDERED PROPERLY. PDF contains only font definitions and basic structure, no actual Arabic content. This means users see boxes or garbled characters instead of proper Arabic text like 'أمر شراء', 'رقم الأمر', 'المشروع', 'المورد', etc. The html2pdf.js library is not handling Arabic text rendering correctly. This is exactly the issue user reported - PDF generates but Arabic text appears as þ€þ•þ®þ· or boxes."